   ...
   try {
      ...
      if (!m.fromMe && !m.isGroup && !users.banned) {
         if (setting.groupmode && !isOwner && !isPrem) {
            if (!users.whitelist && m.text && (m.text.charAt(0) == myPrefix || Func.socmed(m.text)) && !/owner|premium|sewabot|batu|gunting|kertas/.test(m.text)) return client.sendMessageModify(m.chat, Func.texted('bold', `*Bot Ini Ngak Bisa Dipake Di Private Chat*\nHanya Bisa Digunakan Di Grup\n\nKalau Mau Main Bot, Kk Tinggal Gabung Ke Grup Bot,\nPencet Tulisan Dibawah Ini`), m, {
            title: 'ılılılllıılılıllllıılılllıllı\nMaap Kak, Akses Ditolak...🌷',
            largeThumb: true,
            thumbnail: await Func.fetchBuffer('https://telegra.ph/file/73cb2b865ab3beef12bf1.jpg'),
            url: 'https://chat.whatsapp.com/GLDpzSjcXpZIp007nLaRNy'
         })            
         }
